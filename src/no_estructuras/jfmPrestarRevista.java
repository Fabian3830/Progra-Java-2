...
public class jfmPrestarRevista extends javax.swing.JFrame implements Runnable {

    boolean cliente_encontrado = false;
    boolean revistaEncontrado = false;
    ...
    public void cargar_tabla() {
        ...
        for (int i = 0; i < jfmClientes.clientes.size(); i++) {
            ...
            if (jfmClientes.clientes.get(i).getRevistaPrestado()!= null) {
        ...
        if (aux_para_llenar.size() > 0) {
            ...
            for (int i = 0; i < aux_para_llenar.size(); i++) {
                ...
                matriz[i][1] = aux_para_llenar.get(i).getRevistaPrestado().getTitulo();
                matriz[i][2] = new SimpleDateFormat("dd/MM/yyyy").format(aux_para_llenar.get(i).getRevistaPrestado().getFechaEntrega());
                Date devolucion = aux_para_llenar.get(i).getRevistaPrestado().getFechaDevolucion();
            ...
            tabla_devolucion.setModel(new javax.swing.table.DefaultTableModel(
                    matriz,
                    new String[]{
                        "cedula", "revista", "dia prestado", "fecha devolucion", "devolucion a tiempo"
                    }
            ) {
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("Devolucion de Revista", jPanel2);
        ...
        jTabbedPane1.addTab("Prestar Revista", jPanel1);
    ...
    private void btn_hace_prestamoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_hace_prestamoActionPerformed
        ...
        jfmRevista.revistaLista.get(prestarRevista).setDisponibilidad(false);
        //declaro el tiempo de devolucion 1 minuto despues
         jfmRevista.revistaLista.get(prestarRevista).setFechaDevolucion(calendario.getTime());
         jfmRevista.revistaLista.get(prestarRevista).setFechaEntrega(ya_mismo);
        //PONGO QUIEN TIENE EL LIBRO EN sus cosas prestadas
         jfmRevista.revistaLista.get(prestarRevista).setCedulaPersona(jfmClientes.clientes.get(prestar_cliente));
        //pongo el libro prestado en el CLIENTE QUE LO PIDE
        jfmClientes.clientes.get(prestar_cliente).setRevistaPrestado( jfmRevista.revistaLista.get(prestarRevista));

        System.out.println( jfmRevista.revistaLista.get(prestarRevista).getTitulo()+" DISP" +  jfmRevista.revistaLista.get(prestarRevista).isDisponibilidad());
    ...
    private void btn_devolverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_devolverActionPerformed
        ...
        for (int i = 0; i < jfmClientes.clientes.size(); i++) {
            if (jfmClientes.clientes.get(i).getCedula().equals(criterio_devolver)) {
                d = i;
                encontrado = jfmClientes.clientes.get(i).getRevistaPrestado();
                criterio_devolver = "";
                break;
            }
        }
        ...
        if (d != -1 && encontrado != null) {
            ...
            for (int i = 0; i <  jfmRevista.revistaLista.size(); i++) {
                if (encontrado.getCodigoUnidad().equals( jfmRevista.revistaLista.get(i).getCodigoUnidad())) {

                     jfmRevista.revistaLista.get(i).setDisponibilidad(true);
                    break;
                }
            }

            jfmClientes.clientes.get(d).setRevistaPrestado(null);
    ...
    public void revistaAnadir() {
        ...
        if (!"".equals(codigo_U)) {
            ...
            for (int i = 0; i <  jfmRevista.revistaLista.size(); i++) {
                if ( jfmRevista.revistaLista.get(i).getCodigoUnidad().equals(codigo_U)) {
                    indice = i;
                    encontrado = true;
                }
            }
            if (indice != -1 && encontrado==true) {
                ...
                System.out.println( jfmRevista.revistaLista.get(indice).getTitulo()+" CONDICION UNO " +  jfmRevista.revistaLista.get(indice).isDisponibilidad());
                if ( jfmRevista.revistaLista.get(indice).isDisponibilidad() == true) {
                    Revista a_prestar =  jfmRevista.revistaLista.get(indice);
                    prestarRevista = indice;
    ...
    public void cliente_anadir() {
        ...
        if (!"".equals(cedula_unica)) {
            ...
            if (indice != -1 && encontrado) {
                ...
                if (jfmClientes.clientes.get(indice).getRevistaPrestado()== null) {
    ...
    public void run() {
        ...
        while (true) {
            System.out.print("");
            if (revistaEncontrado && cliente_encontrado) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(jfmPrestarRevista.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(() -> {
            new jfmPrestarRevista().setVisible(true);
        });
    }
