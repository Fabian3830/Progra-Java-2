...
public class jfmPrestarPeriodico extends javax.swing.JFrame implements Runnable {

    boolean cliente_encontrado = false;
    boolean periodicoEncontrado = false;
    ...
    public void cargar_tabla() {
        ...
        for (int i = 0; i < jfmClientes.clientes.size(); i++) {
            ...
            if (jfmClientes.clientes.get(i).getPeriodicoPrestado()!= null) {
        ...
        if (aux_para_llenar.size() > 0) {
            ...
            for (int i = 0; i < aux_para_llenar.size(); i++) {
                ...
                matriz[i][1] = aux_para_llenar.get(i).getPeriodicoPrestado().getTitulo();
                matriz[i][2] = new SimpleDateFormat("dd/MM/yyyy").format(aux_para_llenar.get(i).getPeriodicoPrestado().getFechaEntrega());
                Date devolucion = aux_para_llenar.get(i).getPeriodicoPrestado().getFechaDevolucion();
            ...
            tabla_devolucion.setModel(new javax.swing.table.DefaultTableModel(
                    matriz,
                    new String[]{
                        "cedula", "periodico", "dia prestado", "fecha devolucion", "devolucion a tiempo"
                    }
            ) {
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("Devolucion de Periodico", jPanel2);
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                                    ...
                                    .addComponent(lb_editorial1, javax.swing.GroupLayout.Alignment.LEADING))
                                .addGap(77, 77, 77)
                                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    ...
                                    .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                                        ...
                                        .addComponent(lb_editorial, javax.swing.GroupLayout.Alignment.LEADING))))))
                    .addGroup(jPanel1Layout.createSequentialGroup()
        ...
        jTabbedPane1.addTab("Prestar Periodico", jPanel1);
    ...
    private void btn_hace_prestamoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_hace_prestamoActionPerformed
        ...
        jfmPeriodico.periodico.get(prestaPeriodico).setDisponibilidad(false);
        //declaro el tiempo de devolucion 1 minuto despues
        jfmPeriodico.periodico.get(prestaPeriodico).setFechaDevolucion(calendario.getTime());
        jfmPeriodico.periodico.get(prestaPeriodico).setFechaEntrega(ya_mismo);
        //PONGO QUIEN TIENE EL LIBRO EN sus cosas prestadas
        jfmPeriodico.periodico.get(prestaPeriodico).setCedulaPersona(jfmClientes.clientes.get(prestar_cliente));
        //pongo el libro prestado en el CLIENTE QUE LO PIDE
        jfmClientes.clientes.get(prestar_cliente).setPeriodicoPrestado(jfmPeriodico.periodico.get(prestaPeriodico));

        System.out.println(jfmPeriodico.periodico.get(prestaPeriodico).getTitulo()+" DISP" + jfmPeriodico.periodico.get(prestaPeriodico).isDisponibilidad());
    ...
    private void btn_devolverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_devolverActionPerformed
        ...
        for (int i = 0; i < jfmClientes.clientes.size(); i++) {
            if (jfmClientes.clientes.get(i).getCedula().equals(criterio_devolver)) {
                d = i;
                encontrado = jfmClientes.clientes.get(i).getPeriodicoPrestado();
                criterio_devolver = "";
                break;
            }
        }
        ...
        if (d != -1 && encontrado != null) {
            ...
            for (int i = 0; i < jfmPeriodico.periodico.size(); i++) {
                if (encontrado.getCodigoUnidad().equals(jfmPeriodico.periodico.get(i).getCodigoUnidad())) {

                    jfmPeriodico.periodico.get(i).setDisponibilidad(true);
                    break;
                }
            }

            jfmClientes.clientes.get(d).setPeriodicoPrestado(null);
    ...
    public void periodicoAnadir() {
        ...
        if (!"".equals(codigo_U)) {
            ...
            for (int i = 0; i < jfmPeriodico.periodico.size(); i++) {
                if (jfmPeriodico.periodico.get(i).getCodigoUnidad().equals(codigo_U)) {
                    indice = i;
                    encontrado = true;
                }
            }
            if (indice != -1 && encontrado==true) {
                ...
                System.out.println(jfmPeriodico.periodico.get(indice).getTitulo()+" CONDICION UNO " + jfmPeriodico.periodico.get(indice).isDisponibilidad());
                if (jfmPeriodico.periodico.get(indice).isDisponibilidad() == true) {
                    Periodico a_prestar = jfmPeriodico.periodico.get(indice);
                    prestaPeriodico = indice;
                    periodicoEncontrado = true;
    ...
    public void cliente_anadir() {
        ...
        if (!"".equals(cedula_unica)) {
            ...
            if (indice != -1 && encontrado) {
                ...
                if (jfmClientes.clientes.get(indice).getPeriodicoPrestado()== null) {
    ...
    public void run() {
        ...
        while (true) {
            System.out.print("");
            if (periodicoEncontrado && cliente_encontrado) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            new jfmPrestarPeriodico().setVisible(true);
        });
    }
